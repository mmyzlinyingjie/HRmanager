...
public class EmployeeAction extends ActionSupport {
	...
	public ResultSet search() throws Exception {
		...
		ResultSet rs = employeeService.list(con, pageBean, employee, s_bbirthday, s_ebirthday);
		return rs;
	}
	...
	public String execute() throws Exception {
		...
		try {
			...
			int total = employeeService.count(con, employee, s_bbirthday, s_ebirthday);
	...
	public String delete() throws Exception {
		...
		try {
			con = dbUtil.getCon();
			JSONObject result = new JSONObject();
			int delNums = employeeService.delect(con, delIds);
			if (delNums > 0) {
	...
	public String save() throws Exception {
		...
		try {
			...
			JSONObject result = new JSONObject();
			if (StringUtil.isNotEmpty(employeeId)) {
				saveNums = employeeService.modify(con, employee);
			} else {
				saveNums = employeeService.add(con, employee);
			}
			if (saveNums > 0) {
				result.put("success", "true");
			} else {
